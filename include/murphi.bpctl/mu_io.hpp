...
private:
  ...
  char *name;
public:
   argstr(char *val, const char *n):initialized(FALSE) {
    if (n != NULL) {
      name = new char[strlen(n) + 1];
       strcpy(name, n);
    } else
...
public:
  ...
  //im:simulation
  argnum simulate_prob;
  argbool int_simulate_prob;
  argbool stat_simulate_prob;
...
public:
  ...
  void print_progress(int level, bool forceprint = false, bool endlevel =
		      false, bool change_probability = false, double prob =
		      0.0);
  void print_no_error(int);
