...
#define VERBOSE_FILE    "-pf"	/* Print every time, with lots of detail, on a file */
...
#define PRINT_NUM_PROB_VAR "-prob_var"	/* print probability changings every given number of times */
...
//im: Pmurphik options
#define QUEUE_COMPRESSION_FLAG "-queuecomp"
#define SWAP_LEVEL_FLAG "-swaplevel"
#define FORCE_PRINTING_END_LEVEL "-fpel"
//im: Bpctl verification options
#define SAFETY_ALGO "-safety"
#define COMPRESS_NEXT_FLAG "-comp_next"
#define CTRL_NEXT_STATES_FLAG "-ctrl_next"
#define VERBOSE_NUM "-verbose"
#define DISABLE_EXIT "-calc_prob"
//im: simulation
#define SIMULATE_PROB "-simulate_prob"
#define INT_SIMULATE_PROB "-int_simulate_prob"
#define STAT_SIMULATE_PROB "-stat_simulate_prob"
...
print_num_prob_var(1, "probability changings count"),
...
#endif
debug_sym(FALSE, "debug symmetry"), max_collrate(90, "max collision rate"),	//gdp: initialize cached murphi options
max_level(0, "max bfs level"),
swap_level(FALSE, "swap only at BFS level changes"),
simulate_prob(0, "Make a simulation"),
int_simulate_prob(FALSE, "Make an interactive simulation"),
stat_simulate_prob(FALSE, "Make a statistic simulation"),
fpel(FALSE, "force printing end level"),
queue_compr(FALSE, "compress the queue"),
ctrl_next(FALSE, "control the sum of outgoing probabilities to be 1.0"),
state_compr(FALSE, "compress the next states list"),
disable_exit(FALSE,
	     "calculates the correct probability for the outer probability"),
safety(FALSE, "uses the BF algorithm for the safety verification")
{
...
//gdp: cached murphi options begin
    ...
    if (strncmp(option, MAX_COLLRATE_PREFIX, strlen(MAX_COLLRATE_PREFIX))
	== 0) {
	...
	if (isdigit(temp_str[0])) {
	  ...
	  Error.Notrace
	      ("Unrecognized bound for BPMC.  Do '%s -h' for list of valid arguments.",
	       argv[0]);
      } else {
...
//im: pmurphik options begin

    if (strcmp(option, QUEUE_COMPRESSION_FLAG) == 0) {
      Error.Notrace("Option %s not available in BPMC", option);
      continue;
    };

    if (strcmp(option, SWAP_LEVEL_FLAG) == 0) {
      Error.Notrace("Option %s not available in BPMC", option);
      continue;
    };

    if (strcmp(option, FORCE_PRINTING_END_LEVEL) == 0) {
      fpel.set(true);
      continue;
    };

//im: pmurphik options end

//im: bpctl options begin

    if (strcmp(option, SAFETY_ALGO) == 0) {
      safety.set(true);
      continue;
    };

    if (strcmp(option, CTRL_NEXT_STATES_FLAG) == 0) {
      ctrl_next.set(true);
      continue;
    };

    if (strcmp(option, COMPRESS_NEXT_FLAG) == 0) {
      Error.Notrace("Option %s not available any more", option);
      //state_compr.set (true);
      continue;
    };

    if (strcmp(option, DISABLE_EXIT) == 0) {
      disable_exit.set(true);
      continue;
    };

    if (strcmp(option, VERBOSE_NUM) == 0) {
      if (strlen(option) <= strlen(VERBOSE_NUM)) {	/* We cannot have a space before the number */
	sscanf(options->nextvalue(), "%s", temp_str);
	if (isdigit(temp_str[0])) {
	  sscanf(temp_str, "%lu", &temp);
	  options->next();
	  ...
	  Error.Notrace
	      ("Unrecognized verbose value.  Do '%s -h' for list of valid arguments.",
	       argv[0]);
      } else {
	sscanf(options->value() + strlen(VERBOSE_NUM), "%s", temp_str);
	if (isdigit(temp_str[0]))
	  sscanf(temp_str, "%lu", &temp);
	else
	  Error.Notrace
    ...
    if (strcmp(option, PRINT_NUM_PROB_VAR) == 0) {
      if (strlen(option) <= strlen(PRINT_NUM_PROB_VAR)) {	/* We cannot have a space before the number */
	sscanf(options->nextvalue(), "%s", temp_str);
	if (isdigit(temp_str[0])) {
	  sscanf(temp_str, "%lu", &temp);
	  options->next();
	...
	sscanf(options->value() + strlen(PRINT_NUM_PROB_VAR), "%s",
	       temp_str);
	if (isdigit(temp_str[0]))
	  sscanf(temp_str, "%lu", &temp);
	else
...
//im: bpctl options end


//im: simulation
    if (strcmp(option, SIMULATE_PROB) == 0) {
      if (strlen(option) <= strlen(SIMULATE_PROB)) {	/* We cannot have a space before the number */
	sscanf(options->nextvalue(), "%s", temp_str);
	if (isdigit(temp_str[0])) {
	  sscanf(temp_str, "%lu", &temp);
	  options->next();
	...
	sscanf(options->value() + strlen(SIMULATE_PROB), "%s", temp_str);
	if (isdigit(temp_str[0]))
	  sscanf(temp_str, "%lu", &temp);
	else
	  Error.Notrace
      ...
      simulate_prob.set(temp);
      continue;
    }

    if (strcmp(option, INT_SIMULATE_PROB) == 0) {
      int_simulate_prob.set(TRUE);
      continue;
    };

    if (strcmp(option, STAT_SIMULATE_PROB) == 0) {
      stat_simulate_prob.set(TRUE);
      continue;
    };
...
#ifdef HASHC			// IM: never defined (disabled by --bpctl)
    // added by Uli
    ...
    if (strcmp(option, VERBOSE_FILE) == 0) {
      Error.Notrace("Option %s not available in BPMC", option);
    }
    /* handle trace types. */
    ...
    if (strcmp(option, TRACE_DIFF_FLAG) == 0) {
      ...
      trace_all.set(TRUE);	/* this is just for the check at the end of the function */
      continue;
    }
...
//        }
  ...
  if (fpel.value && !safety.value)
    Error.Notrace("-fpel available only with -safety");
  if (stat_simulate_prob.value && !simulate_prob.value)
    Error.
	Notrace("-stat_simulate_prob available only with -simulate_prob");
  if (trace_all.value && !int_simulate_prob.value)
    Error.
	Notrace("-td and -tf are available only with -int_simulate_prob");
  if (no_verification)
...
void argclass::PrintOptions(void)	// changes by Uli
{
  cout << "Options:\n"
      ...
      << "\t-simulate_prob<n> simulation up to n steps\n"
      << "\t-int_simulate_prob interactive simulation\n"
      <<
      "\t-stat_simulate_prob statistic simulation (only to be used in conjunction with -simulate_prob)\n"
      << "3) Others Options: (default: -m8, -p3, -loop1000)\n" <<
      "\t-m<n>          amount of memory for closed hash table in Mb.\n" <<
      "\t-k<n>          same, but in Kb.\n" <<
      "\t-prob_var<n>   report probability variations every n events.\n" <<
      "\t-p<n>          report progress every 10^n events, n in 1..5.\n" <<
      "\t-pn            print no progress reports (only with -safety).\n"
      <<
      "\t-fpel          force printing at end of level (only with -safety).\n"
      << "\t-verbose<n>    print infos on the algorithm execution.\n" <<
      "\t-calc_prob  computes the exact probability for the outer BPCTL formula.\n"
      << "4) Error Trace Handling: disabled\n" <<
      "\t-td           write only state differences from the previous states (only with -int_simulate_prob).\n"
      <<
      "\t-tf           write full states (only with -int_simulate_prob).\n"
 ...
 ReportManager::print_algorithm()
{
  ...
  if (args->safety.value)
    cout <<
	"\tVerification of a finite horizon safety property by breadth first search.\n";
  else
    cout << "\tVerification of BPCTL property by depth first search.\n";
}
...
void ReportManager::print_progress(int level, bool forceprint,
				   bool endlevel, bool change_probability,
				   double prob)
{
  static bool initialized = FALSE;
  static long last_num_elts = 0;
  static unsigned num_prob_changes = 0;

  if (!change_probability) {
    // pring progress report every <args->progress_count> new states found
    if ((forceprint) ||
	(args->print_progress.value &&
	 StateSet->NumElts() % args->progress_count.value == 0 &&
	 (args->safety.value ? true : last_num_elts !=
	  StateSet->NumElts()))) {
      if (!args->safety.value)
	last_num_elts = StateSet->NumElts();
      if (!initialized) {
	cout << "\nProgress Report:\n\n";
	initialized = TRUE;
      }
      if (endlevel) {
	cout << "\n***************\n";
	cout << "Level " << StateSet->GetCurrentLevel() -
	    1 << " completed.";
      } else {
	cout << "\t";
	cout << StateSet->NumElts() << " states explored in "
      ...
      if (args->safety.value) {
	if (!endlevel)
	  cout << " Exploring level  " << StateSet->
	      GetCurrentLevel() << "." << " Formula probability " <<
	      setprecision(6) << setiosflags(ios::scientific)
	      << E_p << setprecision(3) << resetiosflags(ios::scientific)
	      << ".\n";
	else
	  cout << " Formula probability "
	      << setprecision(6) << setiosflags(ios::scientific)
	      << E_p << setprecision(3) << resetiosflags(ios::scientific)
	      << ".\n***************\n\n";
      } else {
	cout << " Top formula probability: "
	    << setprecision(10) << setiosflags(ios::scientific)
	    << prob << setprecision(3) << resetiosflags(ios::scientific)
	    << " ;\n";
      }
      cout.flush();
    }
  } else if (args->print_num_prob_var.value) {
    if (!initialized) {
      cout << "\nProgress Report:\n\n";
      initialized = TRUE;
    }
    if ((++num_prob_changes) % args->print_num_prob_var.value == 0) {
      cout << /*" Current depth  " << level << */
	  "\tTop formula probability incremented: "
	  << setprecision(10) << setiosflags(ios::scientific)
	  << prob << setprecision(3) << resetiosflags(ios::scientific)
	  << " ;\n";
      if (args->verbose.value > 3)
	cout << "\t\teffectiveness: " << StateSet->
	    subf_cache->effectiveness();
      cout.flush();
      num_prob_changes = 0;
    }
  }
...
/************************************************************/
void ReportManager::print_no_error(int reason)
{
  cout << "\n====================================="
      << "=====================================\n" << "\nResults:\n";
  if (reason == 0)
...
void ReportManager::print_summary(bool prob)
{
  ...
  if (StateSet == NULL) {
	...
	<< SecondsSinceStart() << "s.\n";
    if (!args->safety.value)
      cout << "\tMaximum depth reached: " << Algorithm->
	  max_level << "\n\n";
    else
      cout << "\tLevels explored: " << Algorithm->max_level << "\n\n";
  }
...
void ReportManager::print_fire_startstate()
{
  cout << "Firing startstate " << StartState->LastStateName()
      << " with probability " << StartState->LastStateProb()
      << "\n" << "Obtained state:\n";
  theworld.print();
  cout << '\n';
}
...
/************************************************************/
void ReportManager::print_fire_rule(double prob)
{
...
{
  cout << "Firing rule " << Rules->LastRuleName()
      << " with probability " << Rules->LastRuleProb()
      << '\n' << "Obtained state:\n";
  theworld.print();
  cout << '\n';
}
...
/************************************************************/
void ReportManager::print_fire_rule_diff(state * s, double prob)
{
  cout << "Firing rule " << Rules->LastRuleName()
      << " with probability " << prob << '\n' << "Obtained state:\n";
...
void ReportManager::print_verbose_header()
{
  ...
  if (args->verbose_from_state.value)
    cout <<
	"Verbose controlled option selected.  The following is the detailed progress.\n\n";
  else
    cout <<
...
/************************************************************/
void ReportManager::print_final_report(int reason)
{
  print_no_error(reason);
  print_summary(TRUE);
...
const
    TraceFileManager::Buffer * TraceFileManager::read(unsigned long number)
{
  if (number != inBuf) {
