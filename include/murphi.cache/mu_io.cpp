...
no_deadlock(FALSE, "deadlock detection"), print_options(FALSE, "options printing"),
#if __WORDSIZE == 64
...
//gdp: cached murphi options begin
	...
	if (isdigit(temp_str[0])) {
	  ...
	  Error.Notrace
	      ("Unrecognized collision rate limit.  Do '%s -h' for list of valid arguments.",
	       argv[0]);
      } else {
	sscanf(options->value() + strlen(MAX_COLLRATE_PREFIX), "%s",
	       temp_str);
	if (isdigit(temp_str[0]))
	  sscanf(temp_str, "%lu", &temp);
	else
      ...
      if (temp > 99)
	Error.Notrace
	    ("Invalid collision rate limit.  Valid values are in range 0-99");
...
//gdp: cached murphi options end
    ...
    if (strcmp(option, SIMULATE_FLAG) == 0) {
      ...
      Error.Notrace
	  ("Cache can only be used with BFS in this release -- sorry!");
...
#ifndef HASHC
  if (print_trace.value)
    Error.Notrace
	("Option -tv not available in cached verification without -c.");
#endif
}
...
void ReportManager::print_progress(bool forceprint)
{
  ...
  if ((forceprint) || (args->print_progress.value
	...
	SecondsSinceStart() << "s, with " << Rules->
	NumRulesFired() << " rules fired, " << StateSet->QueueNumElts() <<
	" states in the queue, collision rate " << ((double)
						    StateSet->NumOverwrites
						    () / (double)
						    StateSet->NumElts())
	<< ", current level " << StateSet->CurrentLevel() << ".\n";
    cout.flush();
  }
}
...
void ReportManager::print_summary(bool prob)
{
  ...
  cout << "\nState Space Explored:\n\n"
      ...
      << SecondsSinceStart() << "s.\n"
      << "\tCollision Rate: " << ((double) StateSet->NumOverwrites() /
				  (double) StateSet->NumElts()) << ".\n" <<
